...
- Wording repair commit: `beffeac4bdc7178165888184c318b844addc5295`
- Final repair commit: `7864b293028bf0ed1bc99911a766418437933494`
- Deployed image: `sociobotregistry.azurecr.io/sf-in-class-draft-ticket:7864b293028b`
- Image digest: `sha256:3d92ede427dbe5fec1305b086479cf1f76063940be4d168fdbc262a3f67100e0`
...
| F-1-2 | Kept the designed direct 404 document with shared header, footer, legal links, metadata, icons, and HTTP 404. The final CSS repair also gives its wordmark the same ink color as every other route. | `direct 404 keeps the shared navigation, legal links, and complete metadata`, including the computed wordmark color; [`final live 404`](evidence/polish-4/live-404-final-mobile.png). | <https://in-class-draft-ticket.sociobot.in/not-a-route> returned 404 with the headers in [`live-404-final-headers.txt`](evidence/polish-4/live-404-final-headers.txt). |
...
| F-2-12 | Kept “Page not found” on the SPA and direct 404. | `direct 404 keeps the shared navigation, legal links, and complete metadata`; [`final live 404`](evidence/polish-4/live-404-final-mobile.png). | Direct unknown route returned HTTP 404 and that exact heading. |
...
Final clean clone: `/tmp/in-class-draft-ticket-polish4-final-clean.OWbWhb`, cloned after push at `7864b293028bf0ed1bc99911a766418437933494`.
...
- Final-source local gate: `npm test` passed 15 contracts and 58 browser tests. The clean-repair gate also passed 9 Rust tests, `cargo fmt --check`, `cargo clippy --all-targets -- -D warnings`, `cargo build --release`, and `npm audit --omit=dev --audit-level=high`; the final source change was static 404 CSS only.
...
- Release gate: ACR run `ch1bh` built the listed image. The release check observed the final SHA 20 times on PostgreSQL, verified browser flow/export/delete/rate limiting, restarted the revision, and confirmed the PostgreSQL record survived a real process replacement.
- Cold visual evidence: [`live landing mobile`](evidence/polish-4/live-landing-mobile.png), [`live demo mobile`](evidence/polish-4/live-demo-mobile.png), [`final live 404`](evidence/polish-4/live-404-final-mobile.png), and [`live landing desktop`](evidence/polish-4/live-landing-desktop.png).
- Final live mobile Lighthouse: [JSON report](evidence/polish-4/live-lighthouse-mobile-final.json) — Performance 99, Accessibility 100, Best Practices 100, SEO 100; FCP 1.5s, LCP 1.5s, TBT 0ms, CLS 0.048.
