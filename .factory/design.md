...
The landing first screen is left-heavy: a practical statement and action sit beside the generated paper constellation. On phones, the three decision facts come before the artwork. A ruled live-preview strip cuts into the next section. The demo opens on one completed ticket before showing session controls, so the first viewport contains the product result. How-it-works follows a diagonal three-stop plotted line rather than a feature-card grid. The privacy section resembles the reverse of a paper ticket. No pricing placard appears while there is no paid plan. The 404 page names the error plainly while keeping one disconnected plot point inside the shared site shell.
