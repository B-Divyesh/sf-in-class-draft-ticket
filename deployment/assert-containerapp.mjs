 ...
 * look healthy.
 */
export function assertContainerAppContract(resource, contract) {
...
function main() {
  ...
  if (!resourcePath) {
    throw new Error('usage: node deployment/assert-containerapp.mjs <containerapp-resource.json>');
  }
  const resource = JSON.parse(readFileSync(resourcePath, 'utf8'));
  const contract = JSON.parse(readFileSync(contractUrl, 'utf8'));
  assertContainerAppContract(resource, contract);
